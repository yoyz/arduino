...
void loop_treillis_momentary()
{
  if (MODE == MOMENTARY)
      ...
      else
	{
	  for (uint8_t i=0;i<numKeys;i++)
	    trellis.clrLED(i);
	  trellis.setLED(value1);
	  trellis.setLED(value2);
	  // tell the trellis to set the LEDs we requested
	  trellis.writeDisplay();
...
void loop()
{
  ...
  loop_treillis_latching();
  loop_treillis_momentary();
